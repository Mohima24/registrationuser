...
async function bookingData(){
    try {
        let response = await axios.get(`https://delightful-bull-sweatsuit.cyclic.app/booking/getUserAppointment/`,{
            headers: {
...
function renderfun(data){
    ...
    for(let i=0;i<bookingButton.length;i++){
        bookingButton[i].addEventListener('click',(e)=>{
            window.location.href = `https://delightful-bull-sweatsuit.cyclic.app/video/room/${e.target.dataset.id}`            
        })
    }
